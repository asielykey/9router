...
cp .env.example .env
npm install
PORT=20228 NEXT_PUBLIC_BASE_URL=http://localhost:20228 npm run dev   # dev (webpack, port 20127 by default via next dev)
npm run build && PORT=20228 HOSTNAME=0.0.0.0 npm run start           # production
```
- Bun variants: `npm run dev:bun` / `build:bun` / `start:bun`.
- Default runtime port is **20228** (dashboard at `/dashboard`, API at `/v1`).
